...
class RetentionQuery extends Query{
    ...
    filtersFiltered(filters, outputName,fieldName) {
        this.aggregator('filtered').withF('and')
        let self = this
        each(filters, (filter) => self.with(filter))
        this.withG('thetaSketch', outputName, fieldName)
        return this
    }

    intervalFiltered(interval, outputName, fieldName, filters) {
