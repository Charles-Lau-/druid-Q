...
class Query {
    ...
    split(){
        ...
        if(m().toISOString() < inter[1]){
            return [null, this]
            //let startOfDay = m().tz('Europe/Berlin').startOf('day').toISOString()
            //if(inter[0] == startOfDay)
            //    return [null, this._query]
            //else {
            //    let his = this.clone().intervals(inter[0], startOfDay)
            //    let real = this.clone().intervals(startOfDay, inter[1])
            //    return [his, real]
            //}
        }
        else{
        ...
        for (let field of this.requiredField) {
            if (queryFields.indexOf(field) == -1)
                ...
                if (this._query[field] == undefined || this._query[field] == '' ||
                    ...
                    else if (typeof this._query[field] == 'object' && field != 'intervals' && field.indexOf('dimension') < 0) {
